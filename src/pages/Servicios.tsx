...
import { Home, Building2, Hammer, Settings, TreePine, ArrowUpRight, ShieldCheck, Layers } from 'lucide-react';
    ...
    <div className="bg-dark">
      <section className="relative h-screen flex items-center justify-center overflow-hidden px-6">
        <motion.div style={{ scale: heroScale, opacity: heroOpacity, y: yParallax }} className="absolute inset-0 z-0">
          <motion.div animate={{ x: [20, -20], y: [-10, 10], rotate: [-0.5, 0.5], scale: [1.15, 1.25, 1.15] }} transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }} className="w-full h-full">
        ...
        <div className="relative z-20 w-full max-w-7xl mx-auto flex items-center h-full px-8 pt-48 md:pt-80">
          <motion.div initial={{ opacity: 0, x: -60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }} className="w-full">
            <div className="mb-4 md:mb-6 overflow-hidden">
              <motion.span initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="block font-display text-lg sm:text-xl md:text-3xl lg:text-4xl leading-none text-white tracking-[0.4em] md:tracking-[0.7em] uppercase opacity-40 mb-4">SOLUCIONES</motion.span>
            </div>
            <div className="mb-10 md:mb-16 relative">
              <motion.h1 initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 1, ease: [0.16, 1, 0.3, 1] }} className="font-display text-4xl sm:text-6xl md:text-[8rem] lg:text-[10rem] xl:text-[12rem] leading-[0.8] text-primary tracking-tighter italic">SERVICIOS</motion.h1>
            </div>
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="space-y-8 md:space-y-16">
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto">
          ...
          {services.map((service, idx) => (
            ...
            >
              ...
              <div className="relative z-10 space-y-6 sm:space-y-8">
                ...
                <Link to="/contacto" className="flex items-center gap-3 text-white/40 group-hover:text-primary transition-all text-[8px] sm:text-[10px] uppercase tracking-[0.3em] font-mono">
        ...
        <motion.section initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }} viewport={{ once: true }} className="mt-32 relative group overflow-hidden">
          ...
          <div className="relative flex flex-col lg:flex-row items-center gap-10 sm:gap-16 p-8 sm:p-12 lg:p-24 border border-white/5 backdrop-blur-2xl">
            <div className="flex-1 order-2 lg:order-1 lg:max-w-xl">
              ...
              <p className="text-white/80 font-content text-base sm:text-lg leading-relaxed mb-6 sm:mb-8">Contamos con una línea propia de carpintería arquitectónica respaldada por una de las naves con mayor trayectoria en Puebla. Participamos en proyectos residenciales premium, integrando diseño personalizado, procesos de fabricación avanzados y acabados de excelencia.</p>
              <ul className="grid grid-cols-2 gap-y-4 sm:gap-y-6 gap-x-4 sm:gap-x-8 mb-8 sm:mb-12">
                {[{ label: 'Cocinas Integrales', desc: 'Diseño a medida' }, { label: 'Closets y Vestidores', desc: 'Funcionalidad premium' }, { label: 'Puertas de Acceso', desc: 'Acabados especiales' }, { label: 'Mobiliario Empotrado', desc: 'Optimización de espacio' }].map((item, i) => (
              ...
              <motion.a href="https://wa.me/522223028042" target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.05, backgroundColor: '#25D366' }} whileTap={{ scale: 0.95 }} className="inline-block w-full sm:w-auto px-8 sm:px-12 py-4 sm:py-5 bg-primary text-white font-display text-base sm:text-lg uppercase tracking-widest transition-colors duration-300 text-center">Cotízalo por WhatsApp</motion.a>
            </div>
            <motion.div animate={{ y: [-20, 20], rotate: [-1, 1] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }} className="flex-1 order-1 lg:order-2 relative aspect-[4/3] w-full overflow-hidden">
              <img src={imgCarpinteria} className="w-full h-full object-cover grayscale brightness-75 group-hover:grayscale-0 group-hover:brightness-100 transition-all duration-1000" alt="Architecture Carpentry" />
