...
const services = [
  {
    icon: HardHat,
    title: 'Responsiva de Obra',
    description: 'Asumimos la responsabilidad técnica y legal ante las autoridades competentes durante todo el ciclo del proyecto.',
  },
  {
    icon: ClipboardCheck,
    title: 'Revisión y Validación de Proyectos',
    description: 'Análisis exhaustivo de planos, memorias de cálculo y especificaciones para garantizar su viabilidad y corrección.',
  },
  {
    icon: Eye,
    title: 'Supervisión Técnica de Obra',
    description: 'Presencia activa en sitio para verificar que la ejecución cumpla con los estándares de calidad y especificaciones del proyecto.',
  },
  {
    icon: ShieldCheck,
    title: 'Cumplimiento Normativo',
    description: 'Aseguramos que cada etapa del proyecto cumpla con la normativa vigente: NOM, reglamentos locales y federales.',
  },
  {
    icon: FileText,
    title: 'Gestión de Trámites y Licencias',
    description: 'Gestionamos permisos de construcción, licencias municipales y todos los trámites ante dependencias oficiales.',
  },
  {
    icon: AlertTriangle,
    title: 'Detección y Mitigación de Riesgos',
    description: 'Identificamos y controlamos riesgos estructurales, geotécnicos y operativos antes de que afecten el proyecto.',
  },
  {
    icon: Wrench,
    title: 'Asesoría Técnica Especializada',
    description: 'Soporte experto en ingeniería civil, estructural y arquitectónica en cada fase del desarrollo del proyecto.',
  },
]
  ...
  return (
    <section id="dro" aria-labelledby="dro-heading" className="bg-[#111111] py-24 px-4 overflow-hidden">
      <div ref={headerRef} className={`max-w-4xl mx-auto text-center mb-16 transition-all duration-700 ${headerIn ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <h2 id="dro-heading" className="text-white text-4xl md:text-5xl font-bold leading-tight mb-5">
          DRO — <span className="text-[#C00000]">Dirección Responsable</span><br />de Obra
        </h2>
        ...
        <div className="relative rounded-2xl overflow-hidden border border-[#C00000]/30">
          ...
          <div className="relative px-10 md:px-14 py-10 flex flex-col md:flex-row items-start md:items-center gap-6">
            ...
            <p className="text-white/90 text-base md:text-lg leading-relaxed font-light">
              En <span className="font-bold text-white">BRIXA</span>, cada proyecto está respaldado por{' '}
              <span className="text-[#C00000] font-semibold">Dirección Responsable de Obra (DRO)</span>
              , garantizando seguridad, calidad y cumplimiento normativo desde el diseño hasta la entrega.
            </p>
          </div>
        </div>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          {['Seguridad', 'Confianza', 'Legalidad', 'Experiencia', 'Profesionalismo', 'Calidad'].map((badge) => (
            <span key={badge} className="text-xs font-medium text-[#8A8A8A] border border-white/10 px-4 py-1.5 rounded-full hover:border-[#C00000]/40 hover:text-white transition-all duration-300">
              {badge}
            </span>
          ))}
        </div>
